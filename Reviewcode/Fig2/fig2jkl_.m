...
et=40;st=30; iu=zeros(np,nx);
iu(1,:)=300*(tanh(2*(x/3-5))+1)+100; % Initial conditions of E
iu(2,:)=290*(tanh(2*(flip(x)/3-5))+1)+10; % Initial conditions of S
...
xlabel('x (\mum)');ylabel('time (s)');title('Full');set(gca,'FontSize',15);
axis([0 L 0 max_it*dt]);view(0,90);
set(gca,'XTick',[0 15 30]);set(gca,'YTick',[0 1.25]);
caxis([0,500]);colorbar;
subplot(1,3,2)
mesh(x,t,tup');shading interp;
xlabel('x (\mum)');ylabel('time (s)');title('tQSSAp');set(gca,'FontSize',15);
axis([0 L 0 max_it*dt]);view(0,90);
set(gca,'XTick',[0 15 30]);set(gca,'YTick',[0 1.25]);colorbar;
caxis([0,500]);colormap(flip(magma_white));
subplot(1,3,3)
mesh(x,t,sup');shading interp;
xlabel('x (\mum)');ylabel('time (s)');title('sQSSAp');set(gca,'FontSize',15);
axis([0 L 0 max_it*dt]);view(0,90);
set(gca,'XTick',[0 15 30]);set(gca,'YTick',[0 1.25]);colorbar;
caxis([0,500]);colormap(flip(magma_white));
