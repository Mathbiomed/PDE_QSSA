...
iu{1}=10*tanh(6*(x/30-0.5)).*ones(1,ny)+20; % E
iu{2}=20*ones(nx,ny); % D
iu{3}=500*tanh(100*(y/30-0.5)).*ones(nx,1)+520; % S
...
load('magma')
subplot(1,2,1);
surf(x,y,iu{3}'./iu{2}'); shading interp;colormap(magma);
title(TitleStr{1}); set(gca,'FontSize',15); alpha(0.9);colorbar;
xlabel('x');ylabel('y');
set(gca,'XTick',[0 15 30]);set(gca,'YTick',[0 15 30]);view(0,90);
subplot(1,2,2);
surf(x,y,iu{1}'./iu{2}'); shading interp;alpha(0.9);view(0,90);colorbar;
title(TitleStr{2}); set(gca,'FontSize',15); alpha(0.9);
xlabel('x');ylabel('y');
set(gca,'XTick',[0 15 30]);set(gca,'YTick',[0 15 30]);
...
load('deep.mat')
subplot(1,3,1);
surf(x,y,((su{4})./(su{4}+su{3}))'); shading interp;
title('sQSSAp'); set(gca,'FontSize',15); %alpha(0.9);
set(gca,'XTick',[0 0.5 1]);set(gca,'YTick',[0 0.5 1]);
xlabel('x');ylabel('y');
axis image;view(0,90);clim([0,1]);colorbar;colormap(deep);

subplot(1,3,2);
surf(x,y,((tu{4})./(tu{4}+tu{3}))'); shading interp;
...
subplot(1,3,3);
surf(x,y,((fu{4}+fu{6})./(fu{4}+fu{3}+fu{5}+fu{6}))'); shading interp;
title('Full'); set(gca,'FontSize',15); %alpha(0.9);
set(gca,'XTick',[0 0.5 1]);set(gca,'YTick',[0 0.5 1]);
axis image;view(0,90);clim([0,1]);colorbar;colormap(deep);
xlabel('x');ylabel('y');
set(fig3,'position',[100 100 840 420])
